...
export function RankCard({ rank, username, role, temperature, badges = [], commonAnswers }: RankCardProps) {
  ...
  const commonQuestions = [
    { key: "commits", label: "커밋 수", icon: "🤖", value: commonAnswers?.commits },
    { key: "coffee", label: "마신 커피 잔 수", icon: "☕", value: commonAnswers?.coffee },
    { key: "sleep", label: "수면 시간", icon: "😴", value: commonAnswers?.sleep, unit: "시간" },
    { key: "devTime", label: "개발 시간", icon: "💺", value: commonAnswers?.devTime, unit: "시간" },
  ];
      ...
      <div className="flex items-center justify-between">
        <div className="flex-1">
          <div className="flex items-center gap-2 flex-wrap mb-2">
            ...
            <span className={`px-2 py-0.5 rounded text-xs font-medium border ${getRoleColor(role)}`}>
              {role}
            </span>
          </div>
          {badges.length > 0 && (
